...
class MenuUser 
{
	...
	void allPrint() throws IOException
	{
		...
		customTot= ma.price[iBase] + ((bQ - vTemp3.get(iCak-1).getBaseQ())/5 * 2000) + ((iQ - vTemp3.get(iCak-1).getIngQ())/5 * 1000);
		
		if (cusYN == 89 || cusYN == 121) // cakCustom() 에서 y 를 입력했다면, 즉 커스텀 메뉴가 !존재할 때!
		{	
			if (bQ - vTemp3.get(iCak-1).getBaseQ() < 0)		// 입력된 베이스량이 기존 베이스량보다 작다면
			{
				customTot -= (bQ - vTemp3.get(iCak-1).getBaseQ())/5 * 2000;	// 가격이 감산될 테니 그냥 베이스량 연산을 제거함
			}

			if (iQ - vTemp3.get(iCak-1).getIngQ() <0)		// 입력된 재료량이 기존 재료량보다 적다면
			{
				customTot -= (iQ - vTemp3.get(iCak-1).getIngQ())/5 * 1000;	// 가격이 감산될 테니 그냥 재료량 연산을 제거함
			}
			...
			for (int k=vCust3.size()-1; k<vCust3.size(); k++)
				...
				System.out.printf("%s: %s %dml + %s %dml → %d 원", vCust1.get(k), basis[vCust2.get(k)], vCust3.get(k).getBaseQ(), vCust3.get(k).getIng(), vCust3.get(k).getIngQ(), customTot);
		}
		
		else if (cusYN == 78 || cusYN == 110) // cakCustom() 에서 n 을 입력했다면, 즉 커스텀 메뉴가 !존재할 때!
		{
			// 공통 베이스를 가진 메뉴를 저장한 객체인 Temp 1, 2, 3 을 통해 전과 같은 방식으로 메뉴 출력
			customTot = ma.price[iBase];

			System.out.printf("%s: %s %dml + %s %dml → %d 원", vTemp1.get(iCak-1), basis[vTemp2.get(iCak-1)], vTemp3.get(iCak-1).getBaseQ(), vTemp3.get(iCak-1).getIng(), vTemp3.get(iCak-1).getIngQ(), customTot); // iCak 번호가 1을 입력하면 0번째에 있는 값을 출력하도록 해야 돼서 -1 해 줌
		}
	...
	*/
	...
	public static void main(String[] args) throws IOException
	{
		...
		/*
		int iBase = mu.baseSel();
		mu.menuPrint(iBase);
		int iCak = mu.cakSelect();
		mu.cakCustom(iCak);
		*/
	}
}
