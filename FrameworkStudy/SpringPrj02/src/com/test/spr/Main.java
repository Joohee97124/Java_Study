...
public class Main
{
	public static void main(String[] args)
	{
		...
		// 1번 유형의 Record 객체 생성
		Record rec1 = new RecordImpl1();			//-- denpendency (의존 객체)
		
		
		// ② RecordImpl2 (Record) 객체 생성 과정
		// 2번 유형의 Record 객체 생성
		Record rec2 = new RecordImpl2();			//-- denpendency (의존 객체)
		...
		*/
		...
		// jar 파일로 추가해놔서 사용할 수 있음
		// 스프링컨테이너에게 : 여기에 객체들 넣어놓을테니까 너가 꺼내서 읽어보고 처리해주셈,,
		// 작업지시서 읽어주기x 메일 보냈으니까 메일보고 해놔~
		// (원래는 우리가 넣어주고 입력했음,,)
		// " " 로부터 읽어와서 context를 구성하겠다는 의미
		
		// 『getBean()』 메소드를 이용하여 사용자가 원하는 객체를 얻는다
		// 메소드 인자값으로 id 속성값을 지정하게 된다
		// applicationContext.xml의 bean의 id를 ""에 넣어줘
		// Bean으로 얻어지는 값 모두 Object => 다운캐스팅필요
