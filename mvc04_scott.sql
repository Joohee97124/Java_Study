...
, DEPARTMENTNAME    VARCHAR2(30)    -- 부서명
...
, CONSTRAINT EMPLOYEE_REGIOID_FK FOREIGN KEY(REGIONID)
...
--==>>
/*
EMPLOYEEID NAM     SSN      BIRTHDAY   LUNAR  TELEPHONE    DEPARTMENTID  POSITIONID  REGIONID BASICPAY  EXTRAPAY 
------------------------------------------------------------------------------------------------------------------
...
SELECT D.DEPARTMENTID, D.DEPARTMENTNAME
    , (SELECT COUNT(*)
        ...
        WHERE DEPARTMENTID = D.DEPARTMENTID) AS DELCHECK
FROM DEPARTMENT D;
--==>> View DPARTMENTVIEW이(가) 생성되었습니다.


--○ 직위별 최소 기본급 검색 쿼리문 구성
SELECT MINBASICPAY          -- 기본급
FROM POSITION
...
UPDATE EMPLOYEE
SET SSN1 = SUBSTR( CRYPTPACK.DECRYPT(SSN,'971006234567'), 1, 6)
    , SSN2 = CRYPTPACK.ENCRYPT(SUBSTR( CRYPTPACK.DECRYPT(SSN,'971006234567'), 7, 7)
                              , SUBSTR( CRYPTPACK.DECRYPT(SSN,'971006234567'), 7, 7) );
--==>> 1 행 이(가) 업데이트되었습니다.
...
ORDER BY E.EMPLOYEEID;
--==>> View EMPLOYEEVIEW이(가) 생성되었습니다.
-- 보여지는 뷰이니까 SSN2 는 넣지 X ! (어차피 암호화되서 보여지니까)
...
--② 관리자 로그인 (ID→ EMPLOYEID, PW→ SSN2 GRADE= 0)
SELECT NAME
FROM EMPLOYEE
WHERE EMPLOYEEID=2
 AND SSN2 = CRYPTPACK.ENCRYPT('1234567', '1234567')
...
--==>> 조회결과 없음. 관리자 로그인 실패
-->> 한줄구성
SELECT NAME FROM EMPLOYEE WHERE EMPLOYEEID=2 AND SSN2 = CRYPTPACK.ENCRYPT('1234567', '1234567') AND GRADE=0
;
...
SELECT EMPLOYEEID, NAME, SSN, BIRTHDAY
    ...
    , REGIONID, REGIONNAME
    , BASICPAY, EXTRAPAY, GRADE
FROM EMPLOYEEVIEW
ORDER BY EMPLOYEEID;
-->> 한줄 처리
SELECT EMPLOYEEID, NAME, SSN, BIRTHDAY, LUNAR, LUNARNAME, TELEPHONE, DEPARTMENTID, DEPARTMENTNAME, POSITIONID, POSITIONNAME, REGIONID, REGIONNAME, BASICPAY, EXTRAPAY, GRADE FROM EMPLOYEEVIEW ORDER BY EMPLOYEEID
;
...
-->>
SELECT * FROM DEPARTMENTVIEW ORDER BY DEPARTMENTID
;
...
--○ 부서 데이터 수정
UPDATE DEPARTMENT
SET DEPARTMENTNAME = '개발'
WHERE DEPARTMENTID=1;
-->> 한줄구성
UPDATE DEPARTMENT SET DEPARTMENTNAME = '개발' WHERE DEPARTMENTID=1
;
...
WHERE REGIONID=2;
-->> 한줄 구성
UPDATE REGION SET REGIONNAME='경기도' WHERE REGIONID=2
;
...
--○ 직위데이터 수정 쿼리문 구성
UPDATE POSITION
SET POSITIONNAME='경기도'
WHERE POSITIONID=2;
--> 한줄구성
UPDATE POSITION SET POSITIONNAME='경기도' WHERE POSITIONID=2
;
