...
public class Test162
{
	...
	{
		// Set<String> dogs = new Set<String>(); 인터페이스라 안대~~
		Set<String> dogs = new HashSet<String>(); //-- Set implement하고있는 클래스로 객체 생성 (업캐스팅)
		...
		System.out.println();
		//--==>> 기존 데이터 set : 기존 데이터 Set : 리트리버 푸들 시바견 치와와
		...
		dogs.add("말티즈");
		dogs.add("똥개");
		dogs.add("똥개");			// ※ 중복 데이터를 추가한다고 해서
		dogs.add("똥개");			//    데이터 삽입 과정에서 
		dogs.add("똥개");			//    오류가 발생하는 것은 아니다! 
		dogs.add("똥개");
		dogs.add("말티즈");
		dogs.add("말티즈");
		dogs.add("말티즈");
		...
		//	  하나만 유효한 데이터로 구성되어 있는 것을 확인 할 수 있다.
		//	  (아무리 추가해도 한 번만 출력됨)
	}
}