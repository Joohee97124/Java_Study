...
--   어떤 하나의 테이블을... 식별자로 가지는 여러 개의 테이블로
--   나누는 과정을 말한다.
...
가정) 서울여의도 LG 라는 회사에 근무하는 거래처 직원 명단이
      ...
      - 이 때 수행되어야할 쿼리문 → UPDATE 구문
       ...
       -->> 100만 개 행을 하드디스크상에서 읽어다가
            메모리에 로드시켜주어야 한다.
            즉, 100만 * 70Byte 를 모두
            하드디스크상에서 읽어다가 메모릴에 로드시켜 주어야 한다는 말이다.
...
--  - 반드시 고유한 값(데이터)가 들어와야 한다.
--    즉, 중복된 값(데이터)이 없어야 한다.
--    예) 10번이 2개이면 안돼
--  - NULL이 있어서는 안된다. (비어있어서는 안된다.)
--    즉, NOT NULL 이어야한다.

--> 제 1정규화를 수행하는 과정에서
--  부모 테이블의 PRIMARY KEY는
--  항상 자식 테이블의 FOREGIN KEY 로 전이된다.
...
가정) 서울여의도 LG 라는 회사에 근무하는 거래처 직원 명단이
      총 100만 명이라고 가정한다.
      (JOIN 한 1행(레코드)은 90Byte 이다 : 회사ID 각각 1번씩 추가)
       ...
       -->> 1 개 행을 하드디스크상에서 읽어다가
            ...
            --> 이는 테이블의 설계가 잘 된 상황이다.
                ...
                --> 정규화를 수행하기 이전에는 100만 건을 처리해야 할 업무에서
                    ...
                    DB 서버는 메모리 고갈 없이 아주 빠르게 처리 될 것이다.
...
--------------------------------------------------------------------------------
 ...
 HHR7733(한혜림)     TPR234(양파링)      2021-02-04 13:40:50        30 
 LHJ3361(이희주)     CPI110(초파이)      2021-02-05 10:22:30        20
 LHJ3361(이희주)     SWK123(새우깡)      2021-02-06 17:00:20        20 
 LSH7654(이상화)     CPI123(초파이)      2021-02-07 05:00:13        50 
                                    :
                                    :
--------------------------------------------------------------------------------
*/
...
→ 『부서』 테이블과 『사원』 테이블을 JOIN 했을 때의 크기
    (10 * 30Byte) + (1,000,000 * 60Byte) = 300 + 60,000,000 Byte = 60,000,300 Byte
...
→ 『부서』 테이블과 『사원』 테이블을 JOIN 했을 때의 크기
    (500,000 * 30Byte) + (1,000,000 * 60Byte) = 15,000,000 + 60,000,000 Byte = 75,000,000 Byte
...
테이블명 : 사원 → 부모 테이블
-----------------------------------------------------------
사원번호  사원명   주민번호    입사일     급여      직급코드            1사원~10회장
++++++++    
-----------------------------------------------------------
...
---------------------------------------------
           2021-04-05   판매완료
                        판매예정 / 판매중 / 판매완료
                         → 혼란만 야기 (판매일이 있으면 판매완료야)

-----------------------
   생년월일    나이
-----------------------
→ 진짜문제되는구조^^ (나이는 시간이 흘러도 항상 그대로야, 아니면 매년 업데이트필요) / 나이를 없애야함
  1992-09-09   30
...
2. 개체 무결성
    ...
    유일성 을 보장하기 위한 제약조건이다.
    
3. 참조 무결성
    참조 무결성은 릴레이션 간의 데이터 일관성을
    보장하기 위한 제약조건이다.
-->> 컬럼1에서 값이 10 이었으면 다른테이블 컬럼1에서도 10이어야함
...
INSERT INTO TBL_TEST1(COL1, COL2) VALUES(NULL, 'STDUY'); -- → 에러발생
...
--==>>
/*
HR	SYS_C007035	P	TBL_TEST1					ENABLED	NOT DEFERRABLE	IMMEDIATE	VALIDATED	GENERATED NAME			21/04/05	HR	SYS_C007035		
*/
...
HR	SYS_C007035	TBL_TEST1	COL1	1
*/
...
--==>>
/*
HR	SYS_C007035	TBL_TEST1	P	COL1
    ------------
...
INSERT INTO TBL_TEST2(COL1, COL2) VALUES(NULL, 'STDUY'); -- → 에러발생
...
-->> COL1, COL2 는 한꺼번에 묶여서 PK라서 둘중 하나라도 다른 값이면 들어갈 수 있음
...
INSERT INTO TBL_TEST3(COL1, COL2) VALUES(NULL, 'STDUY'); -- → 에러발생
...
--○ PK 지정 실습 (④ 테이블생성 이후 제약조건 추가 → PK 지정)
...
--==>>
/*
HR	SYS_C007039	TBL_TEST5	P	COL1		
HR	SYS_C007040	TBL_TEST5	U	COL2			
*/
...
INSERT INTO TBL_TEST5(COL1, COL2) VALUES(5, 'ABCD');     -- → 에러발생 (UNIQUE 위반)

INSERT INTO TBL_TEST5(COL1, COL2) VALUES(5, NULL);
INSERT INTO TBL_TEST5(COL1, COL2) VALUES(6, NULL);
-->> NULL을 고유값으로 간주하지 않는다

INSERT INTO TBL_TEST5(COL1, COL2) VALUES(NULL, NULL);    -- → 에러발생
INSERT INTO TBL_TEST5(COL1, COL2) VALUES(NULL, 'STDUY'); -- → 에러발생
INSERT INTO TBL_TEST5(COL2) VALUES('STUDY');             -- → 에러발생
...
-- UK지정실습(③ 테이블 생성 이후 제약조건 추가→ UK제약조건 추가)
-- 테이블 생성
