...
--  네 번재 파라미터 값은 몇 번째 등장하는 값을 찾을 것인지에 대한 설정 (1은 생략 가능)
...
-- 최초 '\'가 등장하는 위치
SLECT INSTR(REVERSE(FILENAME), '\' , 1, 1)  -- 마지막 매개변수 1 생략
FROOM TBL_FILES;
...
ORACLE
*/
--> 첫 번째 파라미처 값에 해당하는 문자열을 대상으로
--  왼쪽부터 연속적으로 두 번째 파라미터 값에서 지정한 글자와 같은 글자가 등장할 경우
--  이를 제거한 결과값을 반환한다.
--  단, 완성형으로 처리되지 않는다. (글자 하나씩 또각,,)
...
FROM DUAL;
--==>> 영일영-팔영일구-이오육오
-->> ① 매개변수에 있는 값들을 ②번에 있는 값들 → ③ 번에 있는 값들로 
--   1:1로 (2와3번째 값들을) 매칭해서 바꿔준다
...
--○ ROUND() 반올림을 처리해주는 함수
SELECT 48.678 "1"
    , ROUND(46.678, 2) "2"      -- 소수점 이하 둘째자리까지 표현 (세째자리에서 반올림)
    , ROUND(48.674, 2) "3"
    , ROUND(48.674, 1) "4"
    , ROUND(48.674, 0) "5"
...
--○ TRUNC() 절삭을 처리해주는 함수
SELECT 48.678 "1"
    , TRUNC(48.678, 2) "2"      -- 소수점 이하 둘째자리까지 표현 (세째 자리에서 결삭)
    , TRUNC(48.674, 2) "3"
    , TRUNC(48.674, 1) "4"
    , TRUNC(48.674, 0) "5"
...
SELECT LOG(10,100) "확인1", LOG(10,10) "확인2"
FROM DUAL;
--==>> 2	1
...
--○ SIGN()     서명, 부호, 특징
--> 연산 결과 값이 양수이면 1, 0이면 0, 음수이면 -1을 반환한다.
...
--==>>
/*
2021-03-29 12:04:05	   -- 현재
2021-03-30 12:04:05	   -- 1일 후
2021-03-27 12:04:05	   -- 2일 전
2021-04-01 12:04:05    -- 3일 후
*/
...
--==>>
/*
2021-03-29 12:06:04	-- 현재
2021-03-29 13:06:04	-- 1시간 후
2021-03-29 10:06:04 -- 2시간 전
*/
...
SELECT SYSDATE "현재시간" , SYSDATE + 1 + 2/24 + 3/(24*60) + 4/(24*60*60)
FROM DUAL;
...
2021-06-29 12:29:24 -- 3개월 후
2021-01-29 12:30:13	-- 2개월 전
2020-12-29 12:30:13 -- 3개월 전
*/
--> 월을 더하고 빼기
...
--   첫번째 인자값에 해당하는 날짜보다
--   두번째 인자값에 해당하는 날짜가 『미래』 라는 의미로 확인할 수 있다.
...
SELECT TRUNC(TRUNC(TRUNC(8819482/60)/60)/24)
     ...
     , MOD(8819482, 60)
FROM DUAL;    
--> 아래와 같은 결과 (쿼리문이 실행될때마다 값이 달라지기 때문에
--  이렇게 작성하면 안됨~!
...
SELECT SYSDATE "1"                  --  2021-03-29 → 기본 현재 날짜
    ...
    ,TRUNC(SYSDATE, 'DAY') "5"      --  2021-03-28 → 날짜까지 유효한 데이터 
FROM DUAL;
...
-- 날짜 → 문자
SELECT TO_CHAR(SYSDATE, 'YYYY-MM-DD')       -- 2021-03-29 (날짜→문자)
...
SELECT TO_NUMBER( T0_CHAR(SYSDATE,'YYYY') ) "확인" 
FROM DUAL;
...
SELECT CASE WHEN 3<1 AND 5<2 OR 3>1 AND 2=2 THEN '혜여만세'
            WHEN 5<2 AND 2=2 THEN '새롦만세'
            ELSE '희주만세'
        END "결과 확인"
FROM DUAL;
...
/*
SELECT CASE WHEN F AND T THEN '혜여만세'
            WHEN F THEN '새롦만세'
            ELSE '희주만세'
FROM DUAL;
*/