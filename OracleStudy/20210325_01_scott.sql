...
-- TBS_EDUA라는 테이블스페이스에 만들거야라는 의미

--○ TBS_EXAMPLE1 과 TBL_EXAMPLE2 테이블이
...
EMP	            TABLE	→ EMPLOYESS (사원) 정보 테이블
SALGRADE	    TABLE	→ 급여 (SAL) 정보 테이블
TBL_EXAMPLE1	TABLE	
TBL_EXAMPLE2	TABLE	
*/
...
SELECT *
FROM BONUS;
--==>> ENAME, JOB, SAL, COMM의 컬럼이 있고 데이터는 없다
--==>> 데이터 존재하지 않음
...
--○ 우리가 흔히 웹사이트 등에서 회원 가입을 수행할 경우
--   필수 입력 사항과 선택 입력사항이 있다.
--   필수 입력 항목은 ID, PW, 성형, 주민번호, 전화번호...
--   등과 같은 컬럼이며, 이 값들은 회원가입 절차에 따라
--   반드시 필요한 (존재해야하는) 값이므로 NOT NULL 로 한다.

--   선택 항목은 취미, 결혼여부, 차량소유여부, 특기...
...
1. 숫자형 NUMBER       → -10의 38승 -1 ~ 10의 38승
          ...
          NUMBER(3)    → -999 ~ 999
          NUMBER(4)    → -9999 ~ 9999
          NUMBER(4,1)  → -999.9 ~ 999.9             (4자리 중에 1자리가 실수 형태)
*/
...
--   단, 별칭(ALIAS)을 사용한다.
SELECT EMPNO AS "사원번호", ENAME AS "사원명", JOB AS "직종", SAL AS "급여", DEPTNO AS "부서번호"
FROM EMP
...
--※ 오라클에서... 입력된 데이터(값) 만큼은
--   반.드.시 대·소문자 구분을 한다.
-- 쿼리문은 대소문자 상관X, 값이 대소문자 중요!


--○ EMP 테이블에서 직종이 CLERK 인 사원들 중
--   20번 부서에 근무하는 사원들의
--   사원번호, 사원명, 직종명, 급여, 부서번호 항목을 조회한다.
SELECT EMPNO "사원번호", ENAME "사원명", JOB AS "직종", SAL AS "급여", DEPTNO AS "부서번호"
FROM EMP
...
EMPCOPY1	    SAL	
*/
--> SCOTT 이 가진 여러 테이블들 속 컬럼의 COMMENTS 들이 다 나옴
...
--○ TBL_EMP 테이블에서 모든 사원들의 -> WHERE 안씀 
...
SELECT ENAME "사원명", SAL "급여", COMM "커미션", SAL*12+COMM "연봉"
FROM TBL_EMP;
...
--  첫 번재 파라미터 값이 NULL이 아니면, 그 값 (첫 번재 파라미터) 을 그대로 반환한다.
...
SELECT ENAME "사원명", SAL "급여", COMM "커미션", SAL*12 + NVL(COMM,0) "연봉"
FROM TBL_EMP;
--==>>
/*
SMITH	800		9600
...
SELECT ENAME "사원명", SAL "급여", COMM "커미션", SAL*12 + NVL2(COMM, SAL*12+COMM, SAL*12 ) "연봉"
FROM TBL_EMP;
--==>>
/*
SMITH	800		9600
...
--   NVL() 이나 NVL2() 와 비교하여... 모~~든 경우의 수를 고려할 수 잇는 특징을 갖고 있다.
...
SELECT ENAME "사원명", SAL "급여", COMM "커미션", COALESCE ( (SAL*12+COMM), (SAL*12), COMM, 0 ) "연봉"
FROM TBL_EMP;
--==>>
/*
SMITH	800		9600
