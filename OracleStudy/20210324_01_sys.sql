...
*/
-- DBA_USERS에서 USERNAME 과 ACCOUNT_STATUS 만 가져오겠다
...
FROM DBA_USERS;
-- 모든 항목 (*) 을 다 가져와라 
-- 질의문 (오라클한테 물어봐서 - DBA_USERS에 무슨 항목들이 있니?) → 질의 결과

SELECT USERNAME, USER_ID
FROM DBA_USERS;
-- USERNAME과 USER_ID만 선택적으로 보여줌
...
--※ TABLESPACE 란?
--  세그먼트 (테이블, 인덱스, ...) 를 담아주는 (저장해두는)
--  오라클의 논리적인 저장 구조를 의미한다.

CREATE TABLESPACE TBS_EDUA                      -- CREATE 유형 개체명    → 생성    (이 밑으로 다 옵션)
...
--   물리적인 경로에 'C:\TESTORADATA\TBS_EDUA01.DEF' 생성할 것.
