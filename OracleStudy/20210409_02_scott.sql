...
--> ID와 PW 가 유효하지 않은 데이터로 테스트 한 결과... 데이터 변경 실패

EXEC PRC_STUDENTS_UPDATE('superman','java006$','010-9999-9999','경기일산');
--==>> PL/SQL 프로시저가 성공적으로 완료되었습니다.
--> ID와 PW 가 유효한 데이터로 테스트 한 결과... 데이터 변경 성공
...
-- → 프로시저 호출
EXEC PRC_INSA_INSERT('한혜림', '971006-2234567',SYSDATE , '서울', '010-5555-5555', '영업부', '대리', 50000000, 5000000);
--==>> PL/SQL 프로시저가 성공적으로 완료되었습니다

-- 프로시저 호출로 처리된 결과 확인
SELECT *
FROM TBL_INSA;
--==>> 1061	한혜림	971006-2234567	2021-04-09	서울	010-5555-5555	영업부	대리	50000000	5000000

EXEC PRC_INSA_INSERT('박민지', '960927-2234567',SYSDATE , '서울', '010-6666-6666', '기획부', '대리', 5000000, 500000);
--==>> PL/SQL 프로시저가 성공적으로 완료되었습니다

-- 프로시저 호출로 처리된 결과 확인
SELECT *
FROM TBL_INSA;
--==>> 1062	박민지	960927-2234567	2021-04-09	서울	010-6666-6666	기획부	대리	5000000	500000
...
-- → 프로시저 호출
EXEC PRC_MEMBER_INSERT('정주희','010-1111-1111','광명');
--==>> PL/SQL 프로시저가 성공적으로 완료되었습니다.
EXEC PRC_MEMBER_INSERT('소서현','010-2222-2222','경기');
--==>> PL/SQL 프로시저가 성공적으로 완료되었습니다.
EXEC PRC_MEMBER_INSERT('김동겸','010-3333-3333','인천');
--==>> PL/SQL 프로시저가 성공적으로 완료되었습니다
...
*/
EXEC PRC_MEMBER_INSERT('박민지','010-4444-4444','대전');
--==>> 에러 발생
/*
ORA-20001: 서울,인천,경기만 입력 가능합니다.
*/
...
ALTER TABLE TBL_출고
ADD CONSTRAINT TBL_출고_PK PRIMARY KEY (출고번호);
--==>> Table TBL_출고이(가) 변경되었습니다.

--○ TBL_출고 테이블의 상품코드는 TBL_상품 테이블의 상품코드를 
--   참조할 수 있도록 외래키(FK) 제약조건 지정
ALTER TABLE TBL_출고
ADD CONSTRAINT TBL_출고_FK FOREIGN KEY(상품코드)
...
-- → 프로시저 호출
EXEC PRC_출고_INSERT('H001',10,1200);
--==>> PL/SQL 프로시저가 성공적으로 완료되었습니다.
...
EXEC PRC_출고_INSERT('H002',10,1200);
--==>> 에러 발생
-- ORA-20002: 재고부족~!!!
EXEC PRC_출고_INSERT('H00',70,1200);
--==>> 에러 발생
-- ORA-20002: 재고부족~!!!
